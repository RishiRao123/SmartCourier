...
const CustomDropdown: React.FC<CustomDropdownProps> = ({ options, value, onChange, label, placeholder }) => {
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute z-[100] mt-2 w-full bg-white border border-gray-100 rounded-[24px] shadow-2xl py-3 overflow-hidden animate-in fade-in zoom-in duration-200 origin-top">
          <div className="max-h-60 overflow-y-auto custom-scrollbar">
            {options.map((option) => (
              <div
                ...
                className={`px-6 py-4 text-sm font-bold cursor-pointer transition-all ${
                  value === option.value 
                    ? 'bg-yellow-500 text-[#071a2a]' 
                    : 'text-gray-600 hover:bg-yellow-500 hover:text-[#071a2a]'
                }`}
              >
                {option.label}
              </div>
