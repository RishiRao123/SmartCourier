...
import { Truck, ArrowRight, Mail, KeyRound, Eye, EyeOff, ShieldCheck, ArrowLeft } from "lucide-react";
...
const ForgotPassword: React.FC = () => {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!otp) { toast.error("Please enter the reset code."); return; }
    ...
    <div className="min-h-screen flex font-sans bg-white relative">
      {loading && (
        <div className="fixed inset-0 z-[100] bg-white/80 backdrop-blur-sm flex items-center justify-center">
          <LoadingSpinner message="Cleaning up the mess..." />
        </div>
      )}
      ...
      <div className="hidden lg:flex w-1/2 bg-[#071a2a] flex-col justify-center items-center relative overflow-hidden p-12">
        ...
        <div className="relative z-10 w-full max-w-lg flex flex-col items-center text-center">
          ...
          <p className="text-gray-300 text-lg mb-12 max-w-md">Securely reset your password to regain access to your delivery dashboard and shipments.</p>
        ...
        <div className="w-full max-w-md relative z-10">
          <div className="mb-10 text-center lg:text-left">
            ...
            <h2 className="text-3xl lg:text-4xl font-black text-[#071a2a] mb-3">
              {step === "email" ? "Password" : "Reset"} <span className="text-yellow-500">{step === "email" ? "Recovery" : "Password"}</span>
            </h2>
          ...
          {(step === "otp" || step === "newPassword") && (
            <form onSubmit={handleResetPassword} className="space-y-6">
              <div>
                <label className="block text-xs font-black text-gray-400 mb-2 uppercase tracking-widest">Reset Code</label>
                <input
                  type="text"
                  maxLength={6}
                  value={otp}
              ...
              {step === "newPassword" && (
                <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-500">
                  <div>
                    <label className="block text-xs font-black text-gray-400 mb-2 uppercase tracking-widest">New Password</label>
                    <div className="relative">
          ...
          <div className="mt-8 text-center">
            <Link to="/login" className="inline-flex items-center gap-2 text-sm font-bold text-gray-400 hover:text-[#071a2a] transition-colors">
