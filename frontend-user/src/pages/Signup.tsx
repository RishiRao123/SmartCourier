...
import { Truck, ArrowRight, ShieldCheck, Eye, EyeOff, KeyRound } from 'lucide-react';
...
const Signup: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/signup', { ...formData, role: 'ROLE_CUSTOMER' });
    ...
    <div className="min-h-screen flex font-sans bg-white relative">
      {loading && (
        <div className="fixed inset-0 z-[100] bg-white/80 backdrop-blur-sm flex items-center justify-center">
          <LoadingSpinner message="Creating your spot..." />
        </div>
      )}
      {verifying && (
        <div className="fixed inset-0 z-[100] bg-white/80 backdrop-blur-sm flex items-center justify-center">
      ...
      <div className="hidden lg:flex w-1/2 bg-[#071a2a] flex-col justify-center items-center relative overflow-hidden p-12">
        ...
        <div className="relative z-10 w-full max-w-lg flex flex-col items-center text-center">
          ...
          <p className="text-gray-300 text-lg mb-12 max-w-md">Join India's fastest growing delivery network. Sign up to enjoy seamless doorstep pickups and real-time tracking.</p>
        ...
        <div className="w-full max-w-md relative z-10">
          <div className="mb-10 text-center lg:text-left">
            <div className="inline-flex items-center gap-2 bg-yellow-50 text-yellow-600 px-4 py-2 rounded-full text-xs font-black uppercase tracking-widest mb-6 lg:hidden">
              <Truck className="w-4 h-4" /> SmartCourier
            </div>
            <h2 className="text-3xl lg:text-4xl font-black text-[#071a2a] mb-3">Create <span className="text-yellow-500">Account</span></h2>
            <p className="text-gray-500 font-medium">Join the SmartCourier network today.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              ...
              {!loading && <ArrowRight className="w-6 h-6 transform group-hover:translate-x-1.5 transition-transform" />}
            </button>
            
            <p className="text-center text-gray-500 mt-8 text-sm font-medium">
              Already have an account? <Link to="/login" className="text-[#071a2a] font-black hover:text-yellow-500 transition-colors">Sign in</Link>
            </p>
          </form>
        </div>
      </div>
      ...
      {showOtpModal && (
        <div className="fixed inset-0 bg-[#071a2a]/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl relative">
            ...
            <h3 className="text-2xl font-black text-center text-[#071a2a] mb-2">Verify Your Email</h3>
            <p className="text-center text-gray-500 mb-8 font-medium">
              We've sent a 6-digit OTP to <br/><span className="text-[#071a2a] font-black">{formData.email}</span>
            </p>
            ...
            <div className="mt-6 text-center">
              <p className="text-sm text-gray-500 font-medium">
                ...
                <button type="button" onClick={handleResendOtp} className="text-[#071a2a] font-black hover:text-yellow-500 transition-colors underline">
                  Resend OTP
                </button>
              </p>
            </div>
